...
# ── Page Config ───────────────────────────────────────────────────────────────
st.set_page_config(page_title="LionCity Traffic Intelligence", page_icon="🚦", layout="wide", initial_sidebar_state="expanded")
...
st.sidebar.markdown('<div class="sid-head">🕐 Time</div>', unsafe_allow_html=True)
hour  = st.sidebar.slider("Hour of Day",  0, 23,  8)
month = st.sidebar.slider("Month",        1, 12,  6)

st.sidebar.markdown('<div class="sid-head">📅 Day</div>', unsafe_allow_html=True)
day_of_week       = st.sidebar.selectbox("Day of Week",        day_options)
is_weekend        = st.sidebar.selectbox("Is Weekend?",        ["No", "Yes"])
is_public_holiday = st.sidebar.selectbox("Is Public Holiday?", ["No", "Yes"])
...
workday_flag      = st.sidebar.selectbox("Workday?",           ["No", "Yes"])

st.sidebar.markdown('<div class="sid-head">🌤️ Weather</div>', unsafe_allow_html=True)
temperature_celsius = st.sidebar.slider("Temperature (°C)",      20,   40,   28)
rainfall_mm_per_hr  = st.sidebar.slider("Rainfall (mm/hr)",       0,   60,    0)
humidity_percent    = st.sidebar.slider("Humidity (%)",           65,  100,   80)
wind_speed_kmh      = st.sidebar.slider("Wind Speed (km/h)",       0,   25,   10)
air_quality_index   = st.sidebar.slider("Air Quality Index",      35,   70,   50)

st.sidebar.markdown('<div class="sid-head">🧳 Tourism</div>', unsafe_allow_html=True)
tourist_arrival_idx = st.sidebar.slider("Tourist Arrival Index",  0.0,  1.0,  0.5, step=0.01)

st.sidebar.markdown('<div class="sid-head">🎉 Event</div>', unsafe_allow_html=True)
special_event_type = st.sidebar.selectbox("Special Event Type", event_options)
